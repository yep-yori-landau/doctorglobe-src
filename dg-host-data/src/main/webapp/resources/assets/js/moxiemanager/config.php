<?php
$s3Client = unserialize($_COOKIE['cms_s3Client']);

// General
$moxieManagerConfig['general.license'] = 'Z6JO-CHVW-OWVQ-2BJ6-KYF6-II2Z-ISCK-ZEMX';
...
// Filesystem
$moxieManagerConfig['filesystem.rootpath'] = 's3://'.$s3Client['name']; //./data/files
$moxieManagerConfig['filesystem.include_directory_pattern'] = '';
$moxieManagerConfig['filesystem.exclude_directory_pattern'] = '/(nothis|notthat)/i';
...
$moxieManagerConfig['basicauthenticator.users'] = array(
    array("username" => "prodio", "password" => "prodio", "groups" => array("administrator"))
);
...
$moxieManagerConfig['amazons3.buckets'] = array(
     ...
     */
    $s3Client['name'] => array(
        'publickey' => $s3Client['key'],
        'secretkey' => $s3Client['pwd'],
        'urlprefix' => $s3Client['urlprefix']
     ),
);
